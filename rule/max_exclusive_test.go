...
func Test_MaxExclusiveValidationError(t *testing.T) {
	...
	for ttIdx, tt := range []struct {
		...
		{
			error:           NewMaxValidationError(ve.SubtypeString, 5, false),
			expectedMessage: "must be less than 5 characters",
		},
		{
			error:           NewMaxValidationError(ve.SubtypeString, 5.1, false),
			expectedMessage: "must be less than 5.1 characters",
		},

		{
			error:           NewMaxValidationError(ve.SubtypeSlice, 5, false),
			expectedMessage: "must have less than 5 items",
		},
		{
			error:           NewMaxValidationError(ve.SubtypeSlice, 5.1, false),
			expectedMessage: "must have less than 5.1 items",
		},

		{
			error:           NewMaxValidationError(ve.SubtypeArray, 5, false),
			expectedMessage: "must have less than 5 items",
		},
		{
			error:           NewMaxValidationError(ve.SubtypeArray, 5.1, false),
			expectedMessage: "must have less than 5.1 items",
		},

		{
			error:           NewMaxValidationError(ve.SubtypeMap, 5, false),
			expectedMessage: "must have less than 5 items",
		},
		{
			error:           NewMaxValidationError(ve.SubtypeMap, 5.1, false),
			expectedMessage: "must have less than 5.1 items",
		},

		{
			error:           NewMaxValidationError(ve.SubtypeNumber, 5, false),
			expectedMessage: "must be less than 5",
		},
		{
			error:           NewMaxValidationError(ve.SubtypeNumber, 5.1, false),
			expectedMessage: "must be less than 5.1",
		},

		{
			error:           NewMaxValidationError(ve.SubtypeInvalid, fakerInstance.Int(), false),
...
func maxExclusiveRuleDataProvider() map[string]*ruleTestCaseData {
	return map[string]*ruleTestCaseData{
		...
		"string of length 3": {
			...
			expectedError:    NewMaxValidationError(ve.SubtypeString, 3, false),
		},
		"*string of length 2": {
			rule:             MaxExclusive(3),
			value:            ptr("ab"),
		...
		"int(3)": {
			...
			value:            3,
			expectedNewValue: 3,
			expectedError:    NewMaxValidationError(ve.SubtypeNumber, 3, false),
		},
		"*int(2)": {
			rule:             MaxExclusive(3),
			value:            ptr(2),
		...
		"slice with 3 items": {
			...
			expectedError:    NewMaxValidationError(ve.SubtypeSlice, 3, false),
		},
		...
		"array with 3 items": {
			...
			expectedError:    NewMaxValidationError(ve.SubtypeArray, 3, false),
		},
		...
		"map with 3 keys": {
			...
			expectedError:    NewMaxValidationError(ve.SubtypeMap, 3, false),
		},
		...
		"complex": {
			...
			expectedError:    NewMaxValidationError(ve.SubtypeInvalid, 3, false),
		},
		"bool": {
			rule:             MaxExclusive(3),
			value:            true,
